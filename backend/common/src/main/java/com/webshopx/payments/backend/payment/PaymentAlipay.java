...
public class PaymentAlipay<C extends ClientInfo<C>> {
    ...
    public PaymentOrderResponse handleFaceToFace(PaymentOrderRequest request, C client, Configuration config) {
        String orderId = client.nextOrderId();
        if (orderId == null) {
        ...
        try {
            ...
            model.setTotalAmount(request.getPrice());
            model.setSubject(request.getSubject());
            model.setProductCode(config.getAlipayFaceToFace().getProduceCode());
            model.setBody(request.getSubject());
            ...
            AlipayTradePrecreateResponse response = ProxySupport.call(
                    ...
                    server.getLogger(),
                    () -> alipayClient.execute(precreateRequest)
            );
            ...
            if (response.isSuccess()) {
                ...
                return new PaymentOrderResponse("face2face", orderId, order.getMoney(), response.getQrCode());
            } else {
    ...
    public PaymentOrderResponse handlePolling(PaymentOrderRequest request, C client, Configuration config) {
        String orderId = client.nextOrderId();
        if (orderId == null) {
            return new PaymentOrderResponse("payment.can-not-create-id");
        }
        ...
        return new PaymentOrderResponse("polling", orderId, order.getMoney(), qrcodeURL);
    }
